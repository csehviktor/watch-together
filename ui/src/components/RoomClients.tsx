    ...
    return(
        <div className="bg-gray-800/10 backdrop-blur-sm border border-gray-700/30 rounded-xl p-4">
            <h3 className="text-secondary font-medium mb-3 flex items-center">
                ...
                Watching { Object.keys(roomState?.clients || {}).length } / { roomState?.settings.max_clients ?? 0 }
            </h3>
            <div className="flex flex-wrap gap-2">
                { Object.entries(roomState?.clients || {}).map(([key, client]: [string, WebsocketClient]) => (
                    <div key={key} className="flex items-center space-x-2 bg-gray-700/40 rounded-lg px-3 py-1.5">
                        <Avatar client={client} width={9} height={9} />
                        <span className="text-md">{key}</span>
                        { roomState?.admin?.username === key && (
